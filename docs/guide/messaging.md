...
Durable delivery is **at least once**. A worker can finish a business operation and crash before acknowledgement. Lease renewal reduces concurrent execution but cannot guarantee exactly-once side effects. In-memory state is lost when the process stops.
