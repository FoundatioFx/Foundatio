...
- **Renewal failure means lost ownership**: `ILock.RenewAsync` throws `LockException` when the lock expired, was released, or is held by another owner. It never recreates the lock. Stop the protected work rather than continuing. A lock is not a fencing token, so work already dispatched elsewhere must be idempotent.
