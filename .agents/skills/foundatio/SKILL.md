...
- AWS automatically coalesces concurrent sends/publishes/deletes with bounded per-destination buffers; completion still requires each broker result. Caller cancellation never cancels a shared batch's other inputs and may leave an Unknown send outcome after dispatch. Provider authors can advertise MaxReceiveBatchSize and ReceiveBatchDelay in TransportCapabilities; the pull loop still holds a concurrency slot until settlement, and other providers default to no receive coalescing delay.
