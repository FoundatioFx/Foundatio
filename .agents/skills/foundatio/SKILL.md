...
- **No "fail without retry" status**: `JobResult` only has `IsSuccess` -- there is no separate status for "this failed but don't retry it." Returning a non-success result (`FailedWithMessage`/`FromException`) from `QueueJobBase.ProcessQueueEntryAsync` (or setting `context.Result` in a `WorkItemJob` handler) causes `AbandonAsync`, which re-queues the entry and eventually dead-letters it after `Retries` is exhausted. For permanent/non-recoverable errors, log the error yourself and return `JobResult.Success` / `SuccessWithMessage(...)` to prevent a pointless retry loop; reserve `FailedWithMessage`/`FromException` for transient errors you actually want retried.
