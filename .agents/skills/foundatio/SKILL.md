...
- **RabbitMQ contracts are versioned**: The [4.2.5 delivery guide](../../../docs/guide/implementations/rabbitmq-delivery-safety.md) accompanies runtime PR Foundatio.RabbitMQ#105 in the #103 → #106 → #104 → #105 → #100 review stack. Check package availability and land the provider dependencies before publishing these contracts. Strict dispatch is opt-in and requires Automatic acknowledgement, a nonempty typed terminal exchange, and no discard. Automatic exhaustion without a destination now retains by default: a breaking change with backlog/backpressure consequences. Default FireAndForget is unchanged.
...
- **RabbitMQ endpoint utility**: `RabbitMQEndpointResolver` preserves client-certificate settings from the supplied factory while forcing strict server identity/trust. Custom server-validation callbacks are rejected. This utility does not add client-certificate options to `RabbitMQMessageBusOptions`; configuration preservation is not live mutual-TLS proof.
