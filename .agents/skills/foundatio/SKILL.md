...
- **In-memory conditional updates**: Expired conditional replace/remove operations return false. Explicit conditional removal and empty-list removal free memory immediately without `ItemExpired`. Rejected sized updates preserve the old entry. `Items` exposes values, honoring `CloneValues`, without affecting LRU.
