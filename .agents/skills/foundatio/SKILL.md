...
- **RabbitMQ contracts are versioned**: The [4.2.5 delivery guide](../../../docs/guide/implementations/rabbitmq-delivery-safety.md) accompanies Foundatio.RabbitMQ#100. Check package availability. Strict dispatch is opt-in and requires Automatic acknowledgement, a nonempty typed terminal exchange, and no discard. Automatic exhaustion without a destination now retains by default: a breaking change with backlog/backpressure consequences. Default FireAndForget is unchanged.
