        ...
        try { & dotnet @arguments > (Join-Path $OutputDirectory "$name.log") 2>&1 }
            ...
            if (-not (Test-Path $resultPath)) {
                $failure = @{
                    ...
                    Options = @{ Engine = $parts[0]; Transport = $parts[1]; Scenario = $w.Scenario; ProducerConcurrency = $w.Producers; ConsumerConcurrency = $w.Consumers; Prefetch = $w.Consumers; DeliveryCopies = $w.Subscribers; PayloadBytes = $w.Payload; BatchSize = $w.Batch; RatePerSecond = 0; MaxOutstanding = 1024 }
                }
