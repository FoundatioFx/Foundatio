...
/// </summary>
public sealed class OrderPlacedHandler(ILogger<OrderPlacedHandler> logger, SampleActivity activity) : IMessageHandler<OrderPlaced>
{
    public Task HandleAsync(IMessageContext<OrderPlaced> context, CancellationToken cancellationToken)
    {
        ...
        activity.EventHandled.TrySetResult();
        return Task.CompletedTask;
    }
}

/// <summary>
/// Handles the <see cref="SendReceipt"/> command — competing consumers process queued work, because
/// Program.cs delivers it with <c>bus.SendAsync</c>.
/// </summary>
public sealed class SendReceiptHandler(ILogger<SendReceiptHandler> logger, SampleActivity activity) : IMessageHandler<SendReceipt>
{
    public Task HandleAsync(IMessageContext<SendReceipt> context, CancellationToken cancellationToken)
    {
        ...
        activity.CommandHandled.TrySetResult();
        return Task.CompletedTask;
    }
}
