...
//        private Func<QueueEntry2<T>, Task> _workerAction;
...
//        public RedisQueue2(ConnectionMultiplexer connection, string queueName = null, int retries = 2, TimeSpan? retryDelay = null, int[] retryMultipliers = null,
...
//        public Task<IEnumerable<T>> GetDeadletterItemsAsync(CancellationToken cancellationToken = new CancellationToken()) {
...
//        public Task StartWorking(Func<QueueEntry2<T>, Task> handler, bool autoComplete = false, CancellationToken cancellationToken = new CancellationToken()) {
//            if (handler == null)
...
//            return Task.Factory.StartNew(() => WorkerLoop(_workerCancellationTokenSource.Token), cancellationToken);
//        }
