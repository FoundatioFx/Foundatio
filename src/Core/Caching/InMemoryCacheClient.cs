    ...
    public class InMemoryCacheClient : MaintenanceBase, ICacheClient {
        ...
        public Task<int> RemoveAllAsync(IEnumerable<string> keys = null) {
            ...
            foreach (var key in keys) {
                ...
                _logger.Trace().Message("RemoveAllAsync: Removing key {0}", key).Write();
        ...
        public Task<int> RemoveByPrefixAsync(string prefix) {
            ...
            try {
                ...
                _logger.Error().Message("Error trying to remove items from cache with this {0} prefix", prefix).Exception(ex).Write();
            }
        ...
        public async Task<CacheValue<T>> GetAsync<T>(string key) {
            ...
            if (cacheEntry.ExpiresAt < DateTime.UtcNow) {
                _logger.Trace().Message("TryGetAsync: Removing expired key {0}", key).Write();
            ...
            try {
                ...
                _logger.Error()
                    .Exception(ex)
                    .Message("Unable to deserialize value \"{0}\" to type {1}", cacheEntry.Value, typeof(T).FullName)
                    .Write();
                return CacheValue<T>.NoValue;
            }
        }
        ...
        public async Task<double> SetIfHigherAsync(string key, double value, TimeSpan? expiresIn = null) {
            ...
            var result = _memory.AddOrUpdate(key, new CacheEntry(value, expiresAt, ShouldCloneValues), (k, entry) => {
                ...
                try {
                    ...
                    _logger.Error().Exception(ex).Message($"Unable to increment value, expected integer type.");
                }
        ...
        private async Task<bool> SetInternalAsync(string key, CacheEntry entry, bool addOnly = false) {
            if (entry.ExpiresAt < DateTime.UtcNow) {
                _logger.Trace().Message("SetInternalAsync: Removing expired key {0}", key).Write();
            ...
            if (addOnly) {
                ...
                _logger.Trace().Message("Added cache key: {key}", key).Write();
            } else {
                _memory.AddOrUpdate(key, entry, (k, cacheEntry) => entry);
                _logger.Trace().Message("Set cache key: {0}", key).Write();
            }
            ...
            if (MaxItems.HasValue && _memory.Count > MaxItems.Value) {
                ...
                _logger.Trace().Message("SetInternalAsync: Removing key {key}", key).Write();
        ...
        public async Task<double> IncrementAsync(string key, double amount = 1, TimeSpan? expiresIn = null) {
            ...
            var result = _memory.AddOrUpdate(key, new CacheEntry(amount, expiresAt, ShouldCloneValues), (k, entry) => {
                ...
                try {
                    ...
                    _logger.Error().Exception(ex).Message("Unable to increment value, expected integer type.").Write();
                }
        ...
        public async Task<TimeSpan?> GetExpirationAsync(string key) {
            ...
            _logger.Trace().Message("GetExpirationAsync: Removing expired key {key}", key).Write();
        ...
        protected override async Task<DateTime> DoMaintenanceAsync() {
            ...
            try {
                ...
                _logger.Error().Message("Error trying to find expired cache items.").Exception(ex).Write();
            }

            foreach (var key in expiredKeys) {
                _logger.Trace().Message("Removing expired key: key={key}", key).Write();
