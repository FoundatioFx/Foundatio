...
namespace Exceptionless.Core.Caching {
    public class AzureCacheClient : ICacheClient {
        ...
        public bool TryGet<T>(string key, out T value) {
            value = default(T);
            try {
                object rawValue = DataCache.Get(key);
                if (rawValue == null)
                    return false;
        ...
        public long Increment(string key, uint amount, DateTime expiresAt) {
            return UpdateCounter(key, (int)amount, expiresAt - DateTime.Now);
        }

        public long Increment(string key, uint amount, TimeSpan expiresIn) {
            return UpdateCounter(key, (int)amount, expiresIn);
        }

        private long UpdateCounter(string key, int value, TimeSpan? expiresIn = null) {
            long newValue = DataCache.Increment(key, value, value);
            if (expiresIn.HasValue)
                SetExpiration(key, expiresIn.Value);
        ...
        public DateTime? GetExpiration(string cacheKey) {
            var item = DataCache.GetCacheItem(cacheKey);
            if (item == null)
                return null;

            return DateTime.Now.Add(item.Timeout);
        }
